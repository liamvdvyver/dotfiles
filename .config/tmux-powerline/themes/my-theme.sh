...
bg_f=$(xrdb -query | grep '^rice\.background\.focused:\s#[a-zA-Z0-9]\{6\}$' | cut -f 2)
fg_f=$(xrdb -query | grep '^rice\.foreground\.focused:\s#[a-zA-Z0-9]\{6\}$' | cut -f 2)
bg_a=$(xrdb -query | grep '^rice\.background\.active:\s#[a-zA-Z0-9]\{6\}$' | cut -f 2)
fg_a=$(xrdb -query | grep '^rice\.foreground\.active:\s#[a-zA-Z0-9]\{6\}$' | cut -f 2)
...
nosep="a black black none separator_disable"

if [ -z $TMUX_POWERLINE_LEFT_STATUS_SEGMENTS ]; then
	TMUX_POWERLINE_LEFT_STATUS_SEGMENTS=(
		"tmux_session_info $bg_a $fg_a $nosep" \
		# "hostname 33 0" \
		#"ifstat 30 255" \
		#"ifstat_sys 30 255" \
